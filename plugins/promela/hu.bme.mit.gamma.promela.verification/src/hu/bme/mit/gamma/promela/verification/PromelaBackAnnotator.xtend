...
class PromelaBackAnnotator {
	
	protected final Component component
	protected final PromelaQueryGenerator promelaQueryGenerator
	protected final extension PromelaArrayParser arrayParser
	...
	new(PromelaQueryGenerator promelaQueryGenerator, PromelaArrayParser arrayParser) {
		this.promelaQueryGenerator = promelaQueryGenerator
		this.component = promelaQueryGenerator.component
		this.arrayParser = arrayParser
	}
	
	def void parseState(String potentialStateString, Step step) {
		val instanceState = promelaQueryGenerator.getSourceState(potentialStateString)
	...
	def void parseVariable(String id, String value, Step step) {
		val instanceVariable = promelaQueryGenerator.getSourceVariable(id)
		...
		val field = promelaQueryGenerator.getSourceVariableFieldHierarchy(id)
	...
	def void parseOutEvent(String id, String value, Step step) {
		val systemOutEvent = promelaQueryGenerator.getSourceOutEvent(id)
	...
	def void parseOutEventParameter(String id, String value, Step step) {
		val systemOutEvent = promelaQueryGenerator.getSourceOutEventParameter(id)
		val event = systemOutEvent.get(0) as Event
		val port = systemOutEvent.get(1) as Port
		val systemPort = port.boundTopComponentPort // Back-tracking to the system port
		val parameter = systemOutEvent.get(2) as ParameterDeclaration
		// Getting fields and indexes regardless of primitive or complex types
		val field = promelaQueryGenerator.getSourceOutEventParameterFieldHierarchy(id)
		val indexPairs = id.parseArray(value)
		//
		for (indexPair : indexPairs) {
			val index = indexPair.key
	...
	def void parseSynchronousInEvent(String id, String value, Step step) {
		val systemInEvent = promelaQueryGenerator.getSynchronousSourceInEvent(id)
		if (value == "true" || value == "1") { // For Theta and UPPAAL
	...
	def void parseSynchronousInEventParameter(String id, String value, Step step) {
		val systemInEvent = promelaQueryGenerator.getSynchronousSourceInEventParameter(id)
		val event = systemInEvent.get(0) as Event
		val port = systemInEvent.get(1) as Port
		val systemPort = port.boundTopComponentPort // Back-tracking to the system port
		val parameter = systemInEvent.get(2) as ParameterDeclaration
		// Getting fields and indexes regardless of primitive or complex types
		val field = promelaQueryGenerator.getSynchronousSourceInEventParameterFieldHierarchy(id)
		val indexPairs = id.parseArray(value)
		//
		for (indexPair : indexPairs) {
			val index = indexPair.key
	...
	protected def parseAsynchronousInEvent(String id, String value) {
		val messageQueue = promelaQueryGenerator.getAsynchronousSourceMessageQueue(id)
	...
	def void parseAsynchronousInEventParameter(String id, String value, Step step) {
		val systemInEvent = promelaQueryGenerator.getAsynchronousSourceInEventParameter(id)
		val event = systemInEvent.get(0) as Event
		val port = systemInEvent.get(1) as Port
		val systemPort = port.boundTopComponentPort // Back-tracking to the system port
		...
		if (component.contains(systemPort)) {
			val parameter = systemInEvent.get(2) as ParameterDeclaration
			// Getting fields and indexes regardless of primitive or complex types
			val field = promelaQueryGenerator.getAsynchronousSourceInEventParameterFieldHierarchy(id)
			val indexPairs = id.parseArray(value)
