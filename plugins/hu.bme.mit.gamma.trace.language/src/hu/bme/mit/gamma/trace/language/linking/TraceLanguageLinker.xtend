...
class TraceLanguageLinker extends DefaultLinkingService {
		
    @Inject IValueConverterService valueConverterService
    
    public static extension TracePackage pack = TracePackage.eINSTANCE
    
    override getLinkedObjects(EObject context, EReference ref, INode node) {
    	if (context instanceof ExecutionTrace) {
    		if (ref == executionTrace_Import) {
    			try {
		    		val root = context
