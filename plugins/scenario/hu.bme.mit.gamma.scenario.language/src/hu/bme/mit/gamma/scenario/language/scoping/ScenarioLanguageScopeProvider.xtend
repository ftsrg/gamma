...
class ScenarioLanguageScopeProvider extends AbstractScenarioLanguageScopeProvider {
	...
	override getScope(EObject context, EReference reference) {
		...
		try {
			if (reference == ExpressionModelPackage.Literals.ABSTRACT_DIRECT_REFERENCE_EXPRESSION__DECLARATION) {
			...
			else if (reference == InterfaceModelPackage.Literals.PORT_REFERENCE_EXPRESSION__PORT) {
				...
				return Scopes.scopeFor(package.component.ports)
					...
					InterfaceModelPackage.Literals.EVENT_REFERENCE_EXPRESSION__EVENT) {
				...
				return Scopes.scopeFor(StatechartModelDerivedFeatures.getAllEvents(port))
					...
					ExpressionModelPackage.Literals.ABSTRACT_DIRECT_REFERENCE_EXPRESSION__DECLARATION) {
				val expression = context as EventParameterReferenceExpression
				checkState(expression.port !== null)
