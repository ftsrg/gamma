...
import hu.bme.mit.gamma.constraint.language.validation.ExpressionTypeDeterminator.ExpressionType;
...
class ExpressionTypeDeterminator {
	...
	enum ExpressionType { BOOLEAN, NATURAL, INTEGER, REAL }
	...
	private ExpressionType getArithmeticType(Collection<ExpressionType> collection) {
		...
		if (collection.stream().anyMatch(it -> isNumber(it))) {
			throw new IllegalArgumentException("Type not suitable for arithmetic operations: " + collection);
		}
