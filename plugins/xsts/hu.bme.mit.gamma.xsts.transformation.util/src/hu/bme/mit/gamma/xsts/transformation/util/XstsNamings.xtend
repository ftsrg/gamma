...
class XstsNamings {
	...
	static def String getFlowVariableName(ControlFlow flow) '''activity_control_flow_from_«flow.sourceNode.name»_to_«flow.targetNode.name»_state'''
	static def String getFlowVariableName(DataFlow flow) '''activity_data_flow_from_«ActivityModelDerivedFeatures.getSourceNode(flow).name»_to_«ActivityModelDerivedFeatures.getTargetNode(flow).name»_state'''
